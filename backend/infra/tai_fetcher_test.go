...
func TestTaiFetcher_Get(t *testing.T) {
	t.Run("正常系", func(t *testing.T) {
		...
		for _, path := range paths {
			func(path []string) {
				...
				fetcher := NewTaiFetcher()
				actual, err := fetcher.Get(path[0])
				assert.NoError(t, err)
	...
	t.Run("正常系_該当ファイルが複数存在する場合_最新を返す", func(t *testing.T) {
		...
		fetcher := NewTaiFetcher()
		actual, err := fetcher.Get(testInstallPath)
	...
	t.Run("異常系_該当ファイルなし", func(t *testing.T) {
		...
		for _, path := range paths {
			func(path string) {
				...
				fetcher := NewTaiFetcher()
				_, err = fetcher.Get(testInstallPath)
